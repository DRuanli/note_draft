/**
 * Improved WebSocket Client for Note Management Application
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof ENABLE_WEBSOCKETS !== 'undefined' && ENABLE_WEBSOCKETS) {
        // Initialize NoteWebSocket if the class exists in window
        if (typeof NoteWebSocket === 'undefined') {
            window.NoteWebSocket = class {
                constructor(options = {}) {
                ...
                positionCursorInTextarea(textarea, cursor, position) {
                    ...
                    const cursorLeft = div.offsetWidth + parseInt(window.getComputedStyle(textarea).paddingLeft);
                ...
                // Position cursor in a contenteditable element (simplified)
        ...
        if (typeof USER_ID !== 'undefined') {
            ...
            window.noteWebSocket = new NoteWebSocket({
                ...
                onMessage: function(data) {
                    ...
                    if (data.type === 'note_updated') {
                        ...
                        if (noteEditor && data.content && data.user_id !== USER_ID) {
                        ...
                        if (titleInput && data.title && data.user_id !== USER_ID) {
                        ...
                        if (data.user_id !== USER_ID) {
                            ...
                            // Update collaborators list if function exists
                            if (typeof updateCollaboratorsList === 'function') {
                                if (!window.currentCollaborators) window.currentCollaborators = {};
                                window.currentCollaborators[data.user_id] = data.user_name;
                        ...
                        if (data.user_id !== USER_ID) {
                            ...
                            // Remove from collaborators list if function exists
                            if (typeof updateCollaboratorsList === 'function') {
                                if (window.currentCollaborators && window.currentCollaborators[data.user_id]) {
                                    delete window.currentCollaborators[data.user_id];
                                    updateCollaboratorsList();
                                }
                            }
                            
                            // Remove remote cursor
                            if (window.noteWebSocket.userCursors && window.noteWebSocket.userCursors[data.user_id]) {
                                const cursor = window.noteWebSocket.userCursors[data.user_id];
                                if (cursor.parentNode) {
                                    cursor.parentNode.removeChild(cursor);
                                }
                                delete window.noteWebSocket.userCursors[data.user_id];
                            }
                        }
            ...
            if (noteEditor && noteId) {
                ...
                let lastTitle = noteTitle ? noteTitle.value : null;
                let lastCursorPos = 0;
                ...
                noteEditor.addEventListener('input', function() {
                    ...
                    updateTimeout = setTimeout(function() {
                        if (currentContent !== lastContent) {
                            const titleValue = noteTitle ? noteTitle.value : null;
                            
                            window.noteWebSocket.sendNoteUpdate(noteId, currentContent, titleValue);
                            lastContent = currentContent;
                            if (titleValue !== null) lastTitle = titleValue;
                        }
                    }, 500);
                });
                
                // Handle input changes for title if available
                if (noteTitle) {
                    noteTitle.addEventListener('input', function() {
                        ...
                        updateTimeout = setTimeout(function() {
                            if (currentTitle !== lastTitle) {
                                window.noteWebSocket.sendNoteUpdate(noteId, noteEditor.value, currentTitle);
                                lastTitle = currentTitle;
                                lastContent = noteEditor.value;
                            }
                        }, 500);
                    });
                }
                ...
                noteEditor.addEventListener('select', updateCursorPosition);
...
function showCollaborationToast(message) {
    ...
    const existingToast = document.getElementById('collaborationToast');
    const existingToastBody = document.getElementById('collaborationToastBody');
    
    if (existingToast && existingToastBody) {
        existingToastBody.textContent = message;
